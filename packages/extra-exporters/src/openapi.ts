			...
			for (const t of plan.types) {
				...
				if (t.kind === "struct") {
					...
					if (tAny.obsolete) {
						...
						if (tAny.obsoleteReason) schema.description = (schema.description ? schema.description + " — " : "") + `DEPRECATED: ${tAny.obsoleteReason}`;
					}
					for (const f of t.fields) {
						const fAny = f as any;
						const prop: any = { type: f.type.kind === "primitive" ? "integer" : "string" };
						if (fAny.description) prop.description = fAny.description;
						if (fAny.obsolete) {
							prop.deprecated = true;
							if (fAny.obsoleteReason) prop.description = (prop.description ? prop.description + " — " : "") + `DEPRECATED: ${fAny.obsoleteReason}`;
						}
						schema.properties[f.name] = prop;
					}
					doc.components.schemas[t.name] = schema;
				} else if (t.kind === "enum") {
					const schema: any = { type: "string", enum: t.variants.map((v: any) => v.name) };
					if (tAny.description) schema.description = tAny.description;
					if (tAny.obsolete) {
						schema.deprecated = true;
						if (tAny.obsoleteReason) schema.description = (schema.description ? schema.description + " — " : "") + `DEPRECATED: ${tAny.obsoleteReason}`;
