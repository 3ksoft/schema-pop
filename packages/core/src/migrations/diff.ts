 ...
 *  - "changed": different kinds OR different references — needs hook.
 */
	...
	switch (from.kind) {
		...
		case "array": {
			...
			if (
				...
				return itemCmp === "widened" ? "widened" : "widened";
			}
			return "narrowed";
		}
		case "optional": {
...
function classifyAddedField(field: FieldPlan): {
	...
	if (field.migrationMeta?.defaultValue !== undefined) {
		return {
			default: { kind: "literal", value: field.migrationMeta.defaultValue },
			status: "auto",
		};
	}
	// Primitive without default → language-native default (0 / false / etc.).
	// Auto-resolvable in every target language.
 ...
 * 1. `to.field.migrationMeta.renamedFrom` — explicit rename.
 * 2. Same name (default).
...
function diffStructFields(
	...
	for (let ti = 0; ti < to.fields.length; ti++) {
		...
		const oldName = tf.migrationMeta?.renamedFrom;
		if (!oldName) continue;
	...
	for (let ti = 0; ti < to.fields.length; ti++) {
		const tf = to.fields[ti]!;
		if (tf.migrationMeta?.renamedFrom) continue; // already handled
 ...
 * Pairing rules:
 *  1. If `to.migrationMeta.renamedFrom` is set, pair with that name in `from`.
...
export function diffPlans(from: LayoutPlan, to: LayoutPlan): PlanDiff {
	...
	for (const tt of to.types) {
		const oldName = tt.migrationMeta?.renamedFrom;
		if (!oldName) continue;
	...
	// Pass 2: name-matched
	for (const tt of to.types) {
		if (tt.migrationMeta?.renamedFrom) continue;
