...
import { getSchemaPopConfig, type SchemaPopConfig } from "./schema/schema-pop";
...
export async function buildSchema(
	...
	for (const dest in targetContents) {
		...
		const schemaName = entry.contributors[0]?.schemaName ?? path.basename(dest);
...
function buildMigrationSummary(
	...
	for (const td of diff.types) {
		if (td.kind === "unchanged" || td.kind === "added" || td.kind === "removed")
			continue;
		if (td.status === "auto") {
			autoCount++;
			continue;
