...
function emit(v1: any, v2: any, hooks?: MigrationHooks, cfg: Partial<TsMigrationConfig> = {}) {
...
describe("emitTsMigration", () => {
	test("rename + default + widen → transform body + byte wrapper", () => {
		const v1 = scope({ ...binary.import(), Battery: { voltage_mv: "u32", current: "u8" } });
		const v2 = scope({
			...binary.import(),
			Renamed,
			Battery: {
		...
		expect(code).toContain("export function transformBattery(v1: V1.Battery): V2.Battery");
		...
		expect(code).toContain("export function migrateBattery(v1Bytes: Uint8Array): Uint8Array");
	...
	test("per-field hook → indexes the hooks registry", () => {
		...
		const hooks: MigrationHooks = {
			B: defineMigration<{ x: number }, { x: number }>({ x: (v1) => v1.x & 0xffff }),
		};
	...
	test("whole-type hook → delegates the whole transform", () => {
		...
		const hooks: MigrationHooks = {
			B: defineMigration<{ x: number }, { x: number }>((v1) => ({ x: v1.x & 0xffff })),
		};
		const code = emit(v1, v2, hooks);
