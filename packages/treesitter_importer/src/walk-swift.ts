		...
		function walk(n: TSNode) {
			for (const child of n.namedChildren) {
				...
				if (
					...
					const item = isEnum
						? handleEnum(child, description)
						: handleStruct(child, description);
...
function parseSwiftType(node: TSNode): IRType {
	...
	if (node.type === "optional_type") {
		...
		return {
			kind: "optional",
			inner: inner ? parseSwiftType(inner) : { kind: "unknown", raw: "Any" },
		};
	}
	if (node.type === "array_type") {
		...
		return {
			kind: "array",
			item: item ? parseSwiftType(item) : { kind: "unknown", raw: "Any" },
		};
	}
	if (node.type === "user_type") {
		...
		if (args) {
			...
			if (text === "Optional" && inner)
				return { kind: "optional", inner: parseSwiftType(inner) };
			if (text === "Array" && inner)
				return { kind: "array", item: parseSwiftType(inner) };
		}
	...
	if (text.endsWith("?")) {
		...
		return {
			kind: "optional",
			inner: parseSwiftType({ text: inner, type: "type_identifier" } as TSNode),
		};
	}
	if (text.startsWith("[") && text.endsWith("]")) {
		...
		return {
			kind: "array",
			item: parseSwiftType({ text: inner, type: "type_identifier" } as TSNode),
		};
	}
	return { kind: "ref", name: text };
}
