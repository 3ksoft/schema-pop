...
export interface CConfig extends Omit<BaseConfig, "fieldNaming" | "typeNaming" | "commentStyle"> {
...
export function c(config: CConfig): ExporterPlugin<CConfig> {
	const cfg: CConfig = { fieldNaming: "snake_case", typeNaming: "PascalCase", commentStyle: "star", ...config };
				...
				if (t.kind === "struct") {
					...
					for (const f of t.fields) {
						...
						if (f.bitSize && f.bitSize < 8) {
							...
							const cType = mapScalarField(f.type, C_PRIMITIVES, refName, "uint32_t")!;
