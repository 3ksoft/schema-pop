import type { LayoutPlan, Field, TypePlan, ExporterPlugin, BaseConfig } from "schema-pop";
...
export function random(config: RandomConfig): ExporterPlugin<RandomConfig> {
    ...
    return {
        ...
        generate: (plan: LayoutPlan) => {
            const findType = (name: string): TypePlan | undefined => plan.types.find(t => t.name === name);

            const generateForType = (tp: TypePlan, forceError: boolean): any => {
                if (tp.kind === "struct") {
                    const obj: any = {};
                    for (const f of tp.fields) obj[f.name] = generateValue(f.type, forceError);
                    return obj;
                }
                if (tp.kind === "enum") {
                ...
                if (tp.kind === "union") {
                    ...
                    return typeof payload === "object" && payload !== null ? { kind: v.name, ...payload } : { kind: v.name, value: payload };
                }
            ...
            const generateValue = (f: Field, forceError: boolean): any => {
                ...
                if (f.kind === "primitive") {
                    if (f.name === "bool" || f.name === "boolean") return nextRand() > 0.5;
                ...
                if (f.kind === "string") {
                    ...
                    return Array.from({ length: len }, () => chars[Math.floor(nextRand() * chars.length)]).join("");
                }

                if (f.kind === "array") {
                    const len = f.exactLength ?? Math.floor(nextRand() * (f.maxLength || 5));
                    return Array.from({ length: len }, () => generateValue(f.item, forceError));
                }

                if (f.kind === "optional") {
                    return nextRand() > 0.3 ? generateValue(f.inner, forceError) : undefined;
                }
                ...
                if (f.kind === "inlineStruct") {
                    const obj: any = {};
                    for (const ff of f.fields) obj[ff.name] = generateValue(ff.type, forceError);
                    return obj;
                }
