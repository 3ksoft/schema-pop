...
export interface ZigConfig extends Omit<BaseConfig, "fieldNaming" | "typeNaming" | "commentStyle"> {
...
export function zig(config: ZigConfig): ExporterPlugin<ZigConfig> {
	const cfg: ZigConfig = {
		fieldNaming: "snake_case", typeNaming: "PascalCase", commentStyle: "slash",
		includeComptimeAssertions: true, ...config,
	};
	const { typeName, fieldName, INDENT, mapScalarField, wrapNamespace } = ExporterTools(cfg);

	function fieldZigType(field: Field, fieldSize: number, structAlign: number): string {
	...
	return {
		...
		generate: (plan: LayoutPlan) => {
			let code = "";
			const deprecatedComment = (obsolete?: boolean, reason?: string, indent = "") =>
				obsolete ? `${indent}// DEPRECATED${reason ? `: ${reason}` : ""}\n` : "";
			for (const t of plan.types) {
				...
				if (t.kind === "struct") {
					...
					if (t.fields.length === 0) code += `${INDENT()}_pad: [${t.paddedSize}]u8,\n`;
					let currentBitfieldOffset = -1;
					for (const f of t.fields) {
						if (f.type.kind === "unit") continue;
						const fn = fieldName(f.name);
						const fAny = f as any;
						code += deprecatedComment(fAny.obsolete, fAny.obsoleteReason, INDENT());
						if (f.bitSize && f.bitSize < 8) {
							...
							if (f.paddingAfter > 0) code += `${INDENT()}_pad_${f.offset}: [${f.paddingAfter}]u8,\n`;
						} else {
							const zType = fieldZigType(f.type, f.size, t.align);
							code += `${INDENT()}${fn}: ${zType},\n`;
							if (f.paddingAfter > 0) code += `${INDENT()}_pad_${fn}: [${f.paddingAfter}]u8,\n`;
						}
					}
					code += `};\n\n`;
				} else if (t.kind === "enum") {
