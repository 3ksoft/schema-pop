...
export function go(config: GoConfig = {}): ExporterPlugin<GoConfig> {
	...
	function renderInlineStruct(
		...
		for (const fp of f.fields) {
			...
			s += `${indent(2)}${fn} ${ty}\n`;
			if (fp.paddingAfter > 0)
				s += `${indent(2)}_ [${fp.paddingAfter}]byte\n`;
		}
		s += `${indent()}}`;
		return s;
	}
	...
	function renderUnion(t: UnionPlan, vtn: (n: string) => string): string {
		...
		if (payloadSize > 0)
			code += `${indent()}Payload [${payloadSize}]byte\n`;
		code += `}\n`;
		return code;
	}
	...
	return {
		...
		getHarness: cfg.harness
			? (plans) =>
					goHarness(plans, packageName(plans[0]!), cfg.versionNamespace ?? false)
			: undefined,
	};
	...
	function emitFieldExpr(
		...
		if (change?.kind === "type-widened") {
			const toGo =
				GO_PRIMITIVES[(change.to.type as any).name] ?? "uint32";
