...
async function buildAll() {
    ...
    const harnesses = [
        { name: "rust", dir: "packages/rust", cmd: "cargo build --release && cp target/release/harness ./harness" },
        { name: "cpp",  dir: "packages/cpp",  cmd: "g++ -std=c++17 -O1 src/main.cpp -o harness" },
        { name: "zig",  dir: "packages/zig",  cmd: "zig build-exe src/main.zig -O ReleaseSafe --name harness && rm -f harness.o" },
        { name: "bf",   dir: "packages/bf",   cmd: "gcc -O2 interpreter.c -o bf-interpreter && chmod +x harness" },
        { name: "ts",   dir: "packages/ts",   cmd: "bun build --compile src/run-abi-test.ts --outfile harness" }
    ];
