...
### 1. Better support for type metadata
* Use better typing  for binary/bitwise/reserver/...
...
* **importer corpus regression suite** Run `schema-pop-import` on a large real-world codebase (ESP-IDF the obvious candidate, ~thousands of headers), capture every warning / error / skip-reason, then group + sort to surface the recurring shapes the analyzer can't handle yet (unresolved type names, anonymous nested structs, vendor `Array<T>` aliases, etc.). Add a second pass that re-imports the generated `.ts` outputs through the TypeScript walker — round-trip survival rate becomes a hard CI metric, and any fix to the analyzer / importer can be measured against the corpus instead of one-off tickets. Long run-time, but the resulting data drives priorities.
